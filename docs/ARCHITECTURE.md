...
not implement any of it yet; see [Implementation roadmap](#13-implementation-roadmap)
...
manager, image→ascii, vehicle maintenance log.
...
6. **Low memory** — idle footprint matters; the app is always running.
...
| `%APPDATA%` / system config dirs | Portable by requirement. See §7. |
...
**Exit codes** are the app→launcher channel:
...
2. App notices on next startup or window focus (a poll, not a filesystem watcher —
   there is no urgency and a watcher costs memory).
...
took ~69s. Tools are therefore separate crates, so adding one recompiles that crate
plus the shell rather than the entire dependency graph.
...
- *Bundling a toolchain* — roughly 0.7–1.5 GB with a linker. Fatal to goal 6.
- *Prompting to install rustup* — viable as an on-ramp, ~1 GB and a network
  dependency. Can be offered as a link; not automated.
- *A build server producing custom binaries* — works, but needs infrastructure that
  a single-developer project should not take on.
...
- `rules` — recurrence expansion, guard AST, urgency evaluation (§8). Pure functions
  over a rule and an event history; no clock, no storage.
- `palette` — semantic colour roles, override resolution, contrast validation (§9).
...
**`ToolService`** — created at startup if the tool is enabled. Owns the tool's state,
handles scheduled work and inbound messages, never touches Slint, runs off the UI
thread. **Optional** — a base converter needs no background half and should not get one.

**`ToolView`** — created lazily when the user opens the page, destroyed on navigate
away. Talks to its service over a channel. Only the active view is allocated.
...
shutdown       shutdown hook (§11) → flush → exit
```

Note that **enabled ≠ compiled in**. Everything compiled is present; enabling is a
runtime toggle that controls whether a service runs and whether the tool appears in
the sidebar and in port lookups.
...
2. **Slint** — an aggregate file importing each tool's UI component and mapping tool
   id → component, replacing the hand-written `if current-item == N : XUI {}` chain
...
generated glue wires up. One shared global remains, for theme (§9).
...
1. `data_root` in `config/app.toml`, if the user set one via settings or the first-run
   prompt.
2. Otherwise `<install>/data`.

There is no silent third fallback. If the install directory is not writable — which
happens when installed under `Program Files` without elevation — the app **prompts**
at first run and records the chosen location. It does not quietly relocate data.

Worth warning about at startup: an install folder inside OneDrive/Dropbox can corrupt
a live SQLite file through sync.

### 7.2 Database

`rusqlite` with the `bundled` feature: one file, no external process, ~1 MB, WAL mode
for crash safety. It holds structured data — calendar events, reminder rules, the
event log, health samples, password entries.
...
**Autosave policy** — debounced ~500 ms after a change, plus on navigate-away, on
...
User data is precious. UI state — scroll position, unsaved draft text, selected layer,
expanded panels — is disposable: if it fails to load or its schema drifted, it is
dropped silently. Corrupt UI state must never be able to damage user data.
...
`RRULE` territory — use the `rrule` crate rather than inventing syntax. It covers
...
> = generator `daily at 00:00` + guard `fired(A, within: yesterday)`

This keeps the scheduler dumb — it only ever handles candidate instants — while
...
| `Upcoming` | `soft_start` → `target` | listed quietly in the Upcoming panel |
| `Due` | at `target` | normal notification |
| `Overdue` | `target` → `hard_end` | persistent indicator, intensifying |
| `Critical` | past `hard_end` | prominent, repeating |

It also yields a normalised `0.0..1.0` intensity, which the UI interpolates across
palette roles (`info → warning → error`) so escalation is visible at a glance and
still respects per-tool theming (§9).
...
threshold. The host asks the owning tool "what is current progress toward this task,
`0.0..1.0`?" and escalates on whichever of time or progress is further along. If the
providing tool is missing or disabled, the condition is skipped and time alone
applies — the same graceful degradation as ports (§10).
...
text / background / border families, `success` `warning` `error` `info` `danger`,
...
**The sender never names the receiver.** The image editor asks the host *"who accepts
`luna/image`?"*; the host filters to installed-and-enabled tools and returns a list;
the editor renders a "Send to ▸" submenu from it. If the ascii converter is not
...
because the scheduler is a host service (§8):
...
`Allow | NeedsConfirmation(message) | RequestDelay(reason)` — enough for "file
conversion in progress, ~30s remaining" to hold the door briefly, and for an unsaved
editor to say so.
...
networked features (weather) actually land, rather than paying for it now.
...
1. **Paths + storage + atomic saves.** Portable layout, data-root resolution and
   prompt, SQLite bootstrap, config load/save. Everything else sits on this.
...
**Working:** the workspace split (`luna_lib` / `luna_src`), the base converter
...
- `Global_Calendar_Callback` is not re-exported from `landing_page.slint`, so every
  calendar click handler is unreachable from Rust. Fixed by §6.
...
- `channel_order`: `HLS` → `[H, L, A]` and `HLSA` → `[H, L, A, A]`; both should carry
  `S`.
